...
import { deleteSnapshot } from '../_lib/supabase';
import { requireAppRequest, setNoStore } from '../_lib/security';

const ALLOWED_KEYS = new Set(['confirm']);
...
export default async function handler(
  ...
  if (keys.some((key) => !ALLOWED_KEYS.has(key)) || body.confirm !== true) {
    res.status(400).json({ message: 'Reset confirmation is required.' });
    return;
  }
