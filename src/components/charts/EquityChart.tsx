...
export function EquityChart({ data, className }: EquityChartProps) {
  ...
  const hoveredData = hoveredIndex !== null ? safeData[hoveredIndex] : null;
    ...
    <Card className={className}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-ink-muted">{safeData.length} trading days · {useAccountEquity ? 'Account equity' : 'Trading equity'}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            className={cn(
              'px-3 py-1.5 rounded-lg text-xs font-medium transition-all border',
              useAccountEquity ? 'bg-white/10 text-white border-white/15' : 'bg-white/5 text-ink-muted border-white/10'
            )}
          >
            Account
          </button>
          ...
          {xTickIndices.map((index) => {
            ...
            return (
              <text key={index} x={x} y={CHART_HEIGHT - 12} textAnchor="middle" className="fill-ink-muted text-[10px]">
                {safeData[index]?.date ? formatDate(safeData[index].date) : ''}
              </text>
            );
          })}
        </svg>
