...
export function StrategyExplainer({ className, collapsed = false }: StrategyExplainerProps) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                HIGH: {formatPercent(strategy.highModeRiskPct)} • LOW:{' '}
                {formatPercent(strategy.lowModeRiskPct)} • {strategy.winsToRecover} wins to recover
              </p>
            </div>
          </div>
          <Badge variant={mode === 'HIGH' ? 'high' : 'low'} size="md" pulse>
            {mode}
          </Badge>
        </div>
      </Card>
  ...
  return (
    <Card className={className}>
      <CardHeader
        icon={<ShieldIcon />}
        title={strategy.name}
        subtitle="Adaptive risk management"
      />
      ...
      <div className="mt-5 pt-5 border-t border-white/[0.06]">
        <p className="text-xs text-slate-500">
          <strong className="text-slate-400">How it works:</strong> After a losing trade, risk drops to{' '}
          {formatPercent(strategy.lowModeRiskPct)} to protect your capital. Win{' '}
          {strategy.winsToRecover} trades in a row to return to full{' '}
          {formatPercent(strategy.highModeRiskPct)} risk. This creates a "throttle" that slows down
          during drawdowns and speeds up during winning streaks.
        </p>
      </div>
    </Card>
  );
